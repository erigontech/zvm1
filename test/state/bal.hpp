...
public:
    ...
    ///
    /// 2. System-contract dispatch: the pre-execution loop
    ///    (system_call_block_start) records the address via an explicit
    ///    `state_view.get_account(addr)` BEFORE probing the code, matching
    ///    EELS — an absent EIP-2935/4788 contract still lands in the BAL.
    ///    The post-execution loop (system_call_block_end) probes code first,
    ///    but an empty contract there invalidates the whole block, so the
